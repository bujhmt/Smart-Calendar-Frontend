...
class API {
    ...
    public async login(formData: IAuth): Promise<void> {
        const response: AxiosResponse = await axiosInstance.post(
            '/api/auth/login',
            {...formData, fingerprint: this.fingerprint}
        )
    ...
    public async uploadFiles(data: File | File[]): Promise<any> {
        ...
        const response: AxiosResponse = await axiosInstance.post('/api/media', formData, this.getAxiosConfig())
        if (response.status === 201) {
    ...
    public async updateUserData(): Promise<string> {
        ...
        const response: AxiosResponse = await axiosInstance.put('/api/users/update', this.userData, this.getAxiosConfig())
        if (response.status === 200)
            return response.data.message
    ...
    public async deleteIntentionById(intentionId: string): Promise<string> {
        ...
        const response: AxiosResponse = await axiosInstance.delete(`/api/intentions/${intentionId}`, this.getAxiosConfig())
        if (response.status === 200)
            return response.data.message
        else throw new Error('Please, try again later')
    }
    ...
    public async createIntention(intention: Intention): Promise<string> {
        ...
        const response: AxiosResponse = await axiosInstance.post('/api/intentions/create', intention, this.getAxiosConfig())
