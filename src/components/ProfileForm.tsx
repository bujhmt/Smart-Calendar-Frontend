...
export const ProfileForm = (Props: { setMsg?: React.Dispatch<React.SetStateAction<MsgProps>> }) => {
    ...
    const handleChooseAvatar = useCallback(
        (event: React.ChangeEvent<HTMLInputElement>) => {
            if (event.currentTarget.files) setAva(event.currentTarget.files[0] ?? null)
        },
        [setAva]
    )
    ...
    return (
        <form onSubmit={handleSubmit} className={styles.formWrapper}>
            <input accept="/" id="file-picker" type="file" hidden={true} onChange={handleChooseAvatar} />
            <label htmlFor="file-picker">
                {loading ? (
                    ...
                    <Avatar
                        alt="User Avatar"
                        src={ava ? URL.createObjectURL(ava) : clientApi.userData.avaUrl}
